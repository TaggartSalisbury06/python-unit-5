from flask import Flask, render_template, request, flash, session, redirect
from model import connect_to_db, db
import crud
from jinja2 import StrictUndefined
from fuzzywuzzy import fuzz

app = Flask(__name__)
app.secret_key = "dev"
app.jinja_env.undefined = StrictUndefined

@app.route("/")
def home():
    return render_template("home.html")

@app.route("/users", methods = ["POST"])
def register_user():

    email = request.form.get("email")
    password = request.form.get("password")

    user = crud.get_user_by_email(email)
    if user:
        flash("Cannot create an account with that email. Try again.")
    else:
        user = crud.create_user(email, password)
        db.session.add(user)
        db.session.commit()
        flash("Account created! Please log in.")
    return redirect("/")

@app.route("/movies")
def all_movies():
    movies = crud.get_movies()

    return render_template("all-movies.html", movies=movies)

@app.route("/movies/<movie_id>")
def show_movie(movie_id):

    movie = crud.get_movie_by_id(movie_id)

    return render_template("movie-details.html", movie=movie)

@app.route("/users")
def get_users():
    users = crud.get_users()

    return render_template("all-users.html", users = users)

@app.route("/users/<user_id>")
def get_user_profile(user_id):
    user = crud.get_user_profile(user_id)

    return render_template("user-profile.html", user = user)

@app.route("/login", methods = ["POST"])
def login():
    email = request.form.get("email")
    password = request.form.get("password")

    user = crud.get_user_by_email(email)
    if not user or user.password != password:
        flash("the email or password you entered is incorrect")
    else:
        session["user_email"] = user.email
        flash(f"Welcome back {user.email}!")
    return redirect("/")

@app.route("/update_rating", methods = ["POST"])
def update_rating():
    rating_id = request.json["rating_id"]
    updated_score = request.json["updated_score"]
    crud.update_rating(rating_id, updated_score)
    db.session.commit()

    return "Success"

@app.route("/movies/<movie_id>/ratings", methods = ["POST"])
def create_rating(movie_id):

    logged_in_email = session.get("user_email")
    rating_score = request.form.get("rating")

    if logged_in_email is None:
        flash("You must log in to rate a movie")
    elif not rating_score:
        flash("ERROR: you didn't select a score for your rating")
    else:
        user = crud.get_user_by_email(logged_in_email)
        movie = crud.get_movie_by_id(movie_id)

        rating = crud.create_rating(user, movie, int(rating_score))
        db.session.add(rating)
        db.session.commit()

        flash(f"You rated this movie {rating_score} out of 5.")

    return redirect(f"/movies/{movie_id}")


@app.route("/search")
def search_movies():
    search_query = request.args.get("query")
    movies = crud.get_movies()
    matching_movies = []
    for movie in movies:
        match_ratio = fuzz.token_sort_ratio(search_query, movie.title)
        if match_ratio >= 50:
            matching_movies.append((movie, match_ratio))
    matching_movies = sorted(matching_movies, key=lambda x: x[1], reverse=True)
    movies = [x[0] for x in matching_movies]
    return render_template("search-results.html", movies=movies, search_query=search_query)

if __name__ == "__main__":
    connect_to_db(app)
    app.run(host="0.0.0.0", port = 8000, debug=True)
